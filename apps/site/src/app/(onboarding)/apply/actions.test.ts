...
// Persisted-step authorization is exercised separately by onboarding-access tests.
vi.mock('@/app/(onboarding)/apply/onboarding-access', () => ({
  requireOnboardingStep: vi.fn(async () => ({
    state: { paymentContinued: false },
  })),
}));
import {
...
describe('onboarding action access', () => {
  it.each([
    {
      name: 'invite',
      step: 'team',
    ...
    { name: 'bank setup', step: 'payment', run: () => createAchSetupIntent() },
    {
      name: 'bank confirmation',
      step: 'payment',
      run: () => recordAchSetupComplete('seti_locked'),
    },
    { name: 'terms', step: 'terms', run: () => submitApplication() },
  ])(
    'rejects a stale $name action before changing data',
    async ({ step, run }) => {
      mockGetClaims.mockReturnValue({
        data: { claims: { email: testUserEmail } },
        error: null,
      });
      vi.mocked(requireOnboardingStep).mockRejectedValueOnce(
        new Error('This onboarding step is no longer available.')
      );
      await expect(run()).rejects.toThrow(
        'This onboarding step is no longer available.'
      );
      expect(requireOnboardingStep).toHaveBeenLastCalledWith(step);
