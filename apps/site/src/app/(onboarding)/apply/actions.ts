...
import z from 'zod';
import { isApplicationReadyForSubmission } from '@/app/(onboarding)/apply/db';
import { requireOnboardingStep } from '@/app/(onboarding)/apply/onboarding-access';
import termsAndConditionsVersion from '@/app/(onboarding)/apply/terms-and-conditions-version';
import {
...
export async function submitApplication(): Promise<ActionResponse> {
  ...
  try {
    ...
    await requireOnboardingStep('terms');
...
export async function createAchSetupIntent(): Promise<ActionResponse> {
  try {
    ...
    const onboarding = await requireOnboardingStep('payment');
    if (onboarding.state.paymentContinued)
      throwActionError('Payment setup has already been completed.');
