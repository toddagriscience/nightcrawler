...
describe('rate limit presets', () => {
  ...
  it('configures distinct namespaces so budgets are not shared', async () => {
    mocks.limit.mockResolvedValue({ success: true });
    const { publicWriteRateLimit, publicEmailRateLimit, signupRateLimit } =
      await importFresh();

    await publicWriteRateLimit('1.2.3.4');
...
describe('enforceRateLimit', () => {
  ...
  it('does not throw when the request is under the limit', async () => {
    ...
    // Resolves the client IP from the edge-set header and forwards it.
    expect(mocks.limit).toHaveBeenCalledWith('1.2.3.4');
  });

  it('keys on the right-most forwarded hop, not the spoofable left-most one', async () => {
    mocks.headers.mockResolvedValue(
      new Headers({ 'x-forwarded-for': 'spoofed, 5.6.7.8' })
    );
    mocks.limit.mockResolvedValueOnce({ success: true });
    const { enforceRateLimit } = await importFresh();
  ...
  it('skips the check and warns when no trustworthy client IP is present', async () => {
    mocks.headers.mockResolvedValue(new Headers());
    const { enforceRateLimit } = await importFresh();

    await expect(enforceRateLimit()).resolves.toBeUndefined();
    // No shared "unknown" bucket: header-less callers are not collectively
    // limited, which would be a self-inflicted denial of service.
    expect(mocks.limit).not.toHaveBeenCalled();
    expect(mocks.throwActionError).not.toHaveBeenCalled();
    expect(mocks.loggerWarn).toHaveBeenCalledTimes(1);
  });
