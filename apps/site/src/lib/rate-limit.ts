 ...
 * shared across every limiter, and counters live in Redis rather than in
 * process memory, so a budget is enforced across every serverless instance.
 *
 * Everything fails open: when the Upstash environment variables are missing
 * (local dev, CI, preview deployments), when Redis itself errors, or when the
 * client IP cannot be determined, requests are allowed through so legitimate
 * users are never blocked by infrastructure. `UPSTASH_REDIS_REST_URL` and
 * `UPSTASH_REDIS_REST_TOKEN` are therefore required in production — without
 * them there is no rate limiting at all.
 *
 ...
 * IP per 60 seconds. Reuse this across CMS forms (which now back the contact
 * form) and any other public write action that should share one budget.
 */
...
/**
 * Stricter limiter for public endpoints that send a real email per call, such
 * as resending an approved applicant's activation invite: 3 requests per IP per
 * 10 minutes. Tight enough to make email-bombing an applicant (and burning the
 * Supabase invite quota) impractical, loose enough for a genuine "it never
 * arrived" retry.
 */
export const publicEmailRateLimit = createRateLimiter({
  requests: 3,
  window: '10 m',
  prefix: 'site:public-email',
});

/**
 * Limiter for unauthenticated account creation: 10 requests per IP per 10
 * minutes. Signup is gated only by a token carrying a sequential integer
 * application id, so it needs its own budget rather than sharing the generic
 * public-write one; the allowance leaves room for a legitimate applicant
 ...
 *
 * When no trustworthy client IP is available the check is skipped and a warning
 * is logged, consistent with the limiter's deliberate fail-open stance. Keying
 * such requests under a shared placeholder would instead put every anonymous
 * caller into one bucket and deny service to all of them.
 *
...
export async function enforceRateLimit(
  ...
  if (!ip) {
    logger.warn(
      '[rate-limit] No trustworthy client IP on request; skipping check (failing open).'
    );
    return;
  }
