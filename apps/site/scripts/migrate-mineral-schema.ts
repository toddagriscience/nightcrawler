...
async function inspect() {
  ...
  console.log('mineral_tag enum exists:', tagEnumExists.rows[0].exists);
...
async function migrate() {
  ...
  if (existingCols.has('tag')) {
    ...
    await db.execute(sql`ALTER TABLE mineral ADD COLUMN tag mineral_tag`);
  ...
  if (existingCols.has('four_lows')) {
    ...
    await db.execute(sql`ALTER TABLE mineral ADD COLUMN four_lows boolean`);
