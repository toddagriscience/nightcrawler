...
async function saveToKnowledgeBase(
  ...
  const { db } = await import('@nightcrawler/db/schema/connection');
  const { knowledgeArticle } = await import('@nightcrawler/db/schema');
  const { getEmbedding } = await import('@nightcrawler/db/utils/get-embedding');

  console.log(`  Embedding: "${title}"...`);
  const embedding = await getEmbedding(title + ' ' + markdownContent);

  // knowledgeArticle stores only the embedding; metadata (title/content/category/source)
  // belongs in a specialized table (IMP, seed crop, etc.) that references this row.
  await db.insert(knowledgeArticle).values({ embedding });

  console.log(`  ✓ Saved embedding to knowledge base for: "${title}"`);
}
