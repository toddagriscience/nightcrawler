    ...
    return(
        <ul className="mt-1 space-y-2">
           {Object.entries(linkItem).map(([section, refLinks]) => (
              <li key={section} 
                  className="underline mb-8 flex flex-col gap-2"
              >
                 {refLinks.map((link) => (
                       <Link key={link.label} href={link.href} className="hover:opacity-70 transition duration-200">
                          {link.label}
                       </Link>
                  ))}
              </li>
...
export default async function DisclosuresPage() {
  ...
  const generalLinks: LinkSections = {
    ...
    subLinks: [
      {label: t("links.general.misuseNameBrand"), href: "#"}
    ]
  };

  const ukLinks = [
    {label: t('links.uk.modernSlavery'), href: "#"}
  ];
    ...
    <main className="h-[90vh] mx-auto my-3 max-w-5xl px-6 py-14">
      ...
      <div>
        ...
        <ul className="space-y-2">
           {ukLinks.map((link) => (
              <li key={link.label} className="underline hover:opacity-70 transition duration-200" >
